...
X et al. showed that as time passes (Figure 1), network traffic behavior evolves significantly. Y et al., on the other hand, highlighted the increasing error rates of various anomaly detection models over time if they are not retrained (Figure 2).  
